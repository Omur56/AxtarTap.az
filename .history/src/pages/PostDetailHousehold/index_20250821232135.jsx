...
export default function PostDetailHousehold() {
  ...
  const [zomPhoto, setZoomPhoto] = useState(null);
  ...
  return (
    ...
    <Link to="/Katalog/Məişət_Texnikası"> <button class="flex  items-center gap-2 mt-4 mb-4 bg-gray-200 hover:bg-gray-300 text-gray-800 px-4 py-2 rounded-md">
      ...
      <div className="grid mx-auto max-w-[1000px] grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {[...household].reverse().slice(0, 8).map((item) => (
          <Link key={item._id} to={`/PostDetailHousehold/${item._id}`}>
            <div className="border w-[226px] h-[304px] rounded-lg bg-white shadow-sm hover:shadow-xl transition duration-200">
              ...
              />
              <div className="p-3">
                <h3 className="text-lg font-bold text-red-600">{item.price} AZN</h3>
                <p className="text-sm font-bold truncate w-64">{item.title_typee}</p>
