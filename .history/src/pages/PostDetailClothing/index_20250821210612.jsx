      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-white shadow-md rounded-xl p-4">
        ...
        <div className="space-y-3 mt-[60px]">
          ...
          <ul className="text-xl underline list-inside text-gray-700 space-y-1">
            <li><span className="font-bold">Elan tipi: </span>{post.type}</li>
            ...
            <li><span className="font-bold">Vəziyyəti: </span>{post.condition}</li>
            <li><span className="font-bold">Ölçüsü: </span>{post.size}</li>
            <li><span className="font-bold">Rəngi: </span>{post.color}</li>
            ...
            <li><span className="font-bold">Təsvir: </span>{post.description}</li>
          </ul>
        </div>
          ...
          <Link key={item._id} to={`/PostDetailClothing/${item._id}`}>
            <div className="border sm:w-[240.4px] max-w-[240.4px] h-[300px] rounded-lg bg-white shadow-md hover:shadow-xl transition duration-150">
              <img
                ...
                alt={item.title}
                className="w-full h-[178.5px] object-cover rounded-t-[8px] cursor-pointer"
                onClick={() => openZoom(0)}
              />
              <div className="p-4">
                <h3 className="text-xl font-bold text-black">{item.price} AZN ₼</h3>
                <h2 className="text-sm font-bold truncate w-64">{item.title}</h2>
                <h3 className="text-lg font-semibold truncate w-64">{item.type}</h3>
