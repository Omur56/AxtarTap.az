...
export default function AcsesuarDetail() {
  ...
  return (
    <div className="post-page max-w-6xl  mx-auto p-4">
      ...
      <div className="grid h-[226px] grid-cols-2 sm:grid-cols-2 md:grid-cols-3 min-h-screen lg:grid-cols-4 gap-5">
        {[...accessories].reverse().slice(0, 8).map((item) => (
          <Link key={item._id || item.id} to={`/PostDetailAcsesuar/${item._id || item.id}`}>
            <div className="border w-[] h-[304px] rounded-lg bg-white shadow-sm hover:shadow-xl transition duration-200">
              <img
                src={
