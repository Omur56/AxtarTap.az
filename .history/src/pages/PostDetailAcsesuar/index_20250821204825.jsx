...
export default function AcsesuarDetail() {
  ...
  const [zoomImage, setZoomImage] = useState(null); // zoom üçün state
  ...
  return (
    <div className="post-page max-w-6xl mx-auto p-4">
        ...
        <div className="lg:col-span-2 space-y-6">
          <Carousel showThumbs showStatus={false} autoPlay infiniteLoop>
            {imageArray?.map((img, index) => (
              <div key={index} className="w-full h-[400px] cursor-pointer" onClick={() => setZoomImage(img)}>
                <img
          ...
          <Link key={item._id} to={`/accessory/${item._id}`}>
            <div className="border rounded-lg bg-white shadow-sm hover:shadow-xl transition duration-200">
              <img
                ...
                onClick={() => setZoomImage(item.images?.[0])} // zoom üçün
              />
              <div className="p-3">
      ...
      {zoomImage && (
        ...
        >
          <img
            src={zoomImage.startsWith("http") ? zoomImage : `http://localhost:5000/uploads/${zoomImage}`}
            alt="Zoomed"
