      ...
      <div className="p-4 grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-[15px] w-full">
        {[...cars].map((car) => (
          <Link key={car.id || car._id} to={`/cars/${car.id}`}>
           ...
           {zoomIndex !== null && (
        <div
          className="fixed inset-0 flex  bg-black bg-opacity-90 flex justify-center items-center z-50"
          onClick={closeZoom}
        >
          <div className="relative w-full h-full flex justify-center  text-white items-center">
           {[...cars].map((car) => (
<h2 className="text-lg truncate w-50">
                  {car.category}, {car.brand}, {car.model}
                </h2>
