...
export default function PostDetailCar() {
  ...
  // ✅ Klaviatura ilə şəkil dəyişmə və ESC ilə bağlama
  useEffect(() => {
  ...
  return (
    <div className="max-w-6xl mx-auto p-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 bg-white shadow-lg rounded-xl p-6">
        <div className="lg:col-span-2 space-y-4">
          ...
          <ul className="text-sm text-gray-700 space-y-1 mt-4">
            ...
            <li className="line-clamp-2"><span className="font-bold">Qeyd:</span> {post?.description}</li>
          </ul>
      ...
      {/* Zoom Modal */}
      {zoomIndex !== null && (
        <div className="fixed inset-0 bg-black bg-opacity-90 flex justify-center items-center z-50" onClick={closeZoom}>
          ...
          />
          <button className="absolute right-5 text-white text-3xl font-bold z-50" onClick={(e) => { e.stopPropagation(); nextImage(); }}>›</button>
        </div>
      )}
    </div>
  );
