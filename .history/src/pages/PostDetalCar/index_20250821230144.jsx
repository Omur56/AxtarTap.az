...
export default function PostDetailCar() {
  ...
  // Seçilmiş elan detallarını yüklə
  useEffect(() => {
    axios
    ...
    <div className="max-w-6xl mx-auto p-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 bg-white shadow-lg rounded-xl p-6">
        <div className="lg:col-span-2 space-y-4">
          ...
          <p className="text-3xl font-extrabold text-red-600 mt-4">{post.price} AZN</p>
          ...
          <ul className="text-sm text-gray-700 space-y-1 mt-4">
            ...
            <li>
              <span className="font-bold">Transmissiya:</span> {post.transmission}
            </li>
            <li>
        ...
        <div className="bg-gray-50 border rounded-xl shadow-md p-5 h-fit">
          ...
          <div className="space-y-2 text-gray-700">
            ...
            <p>
              <span className="font-semibold">Email:</span> {post?.contact?.email}
            </p>
            <p>
        ...
        <h2 className="text-[22px] font-bold text-gray-700 mt-10 mb-4"> Bənzər elanlar </h2> <div className="p-4 grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-[15px] w-full"> 
          {[...cars].map((car) => ( <Link key={car.id || car._id} to={/cars/${car.id}}> <div className="bg-white rounded-2xl sm:w-[240.4px] max-w-[240.4px] h-[300px] shadow-md hover:shadow-xl transition duration-200"> <div className="w-full h-[178.5px] bg-gray-100 relative"> <img src={car.images?.[0]?.startsWith("http") ? car.images[0] : "/no-image.jpg"} alt={car.brand} className="absolute top-0 left-0 w-full h-full object-cover rounded-t-2xl" onClick={() => openZoom(0)} /> </div> <div className="p-2"> <h3 className="text-xl font-bold text-black"> {car.price} AZN ₼ </h3> <h2 className="text-lg truncate w-50"> {car.category}, {car.brand}, {car.model} </h2> <p className="text-gray-600 truncate w-64"> {car.year}, {car.km} km </p> <p className="capitalize text-gray-400 text-[14px]"> {car.location}, {formatDate(car.data)}{" "} {getCurrentTime(car.data)} </p> </div> </div> </Link> ))} </div>
      </div>
