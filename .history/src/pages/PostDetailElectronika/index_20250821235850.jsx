...
export default function PostDetailElectronika() {
  ...
  return (
    <div className="post-page max-w-6xl mx-auto p-4">
      <Link to="/Katalog/Elektronika"> <button class="flex  items-center gap-2 mt-4 mb-4 bg-gray-200 hover:bg-gray-300 text-gray-800 px-4 py-2 rounded-md">
        ...
        <div className="bg-gray-50 border rounded-xl shadow-md p-5 h-fit">
          ...
          <div className="space-y-2 text-gray-700">
            ...
            <p><span className="font-semibold">Telefon:</span>
              <a href={`tel:${post?.contact.phone}`} className="text-blue-600 font-bold ml-1">
                {post?.contact.phone}
              </a>
            </p>
          ...
          <a href={`tel:${post?.contact.phone}`} className="text-white font-bold ml-1"> <button className="w-full mt-6 py-3 bg-green-500 hover:bg-red-700 text-white font-bold rounded-lg transition">
      ...
      <div className="grid mx-auto my-auto max-w-[1000px] grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {[...posts].reverse().slice(0, 8).map((item) => (
          <Link key={item._id} to={`/PostDetailElectronika/${item._id}`}>
            <div className="border w-[226px] h-[304px] rounded-lg bg-white shadow-sm hover:shadow-xl transition duration-200">
              ...
              />
              <div className="p-3">
                <h3 className="text-lg font-bold text-red-600">{item.price} ₼</h3>
                <p className="text-sm font-bold truncate w-64">{item.title}</p>
 ...
 <div
          ...
          onClick={closeZoom}
        >
<button className="absolute left-5 text-white text-3xl font-bold z-50" onClick={(e) => {e.stopPropagation();prevImage();}}>
  ‹
</button>

<img src={imageArray[zoomIndex].startsWith("http") ? imageArray[zoomIndex] : `http://localhost:5000/uploads/${imageArray[zoomIndex]}`} alt={`Zoomed in image ${zoomIndex + 1}`} className="max-w-4xl max-h-4xl object-contain` }
