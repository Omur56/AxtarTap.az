...
const [accessories, setAccessories] = useState([]);
 ...
 const [Phone, setPhone] = useState([]);
  ...
  <div className=" p-4 rounded-[4px] grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-[5px] mt-10 w-full  bg-[#ffffff]">
    {[...cars].reverse().map((car) => (
      ...
      >
        ...
        <div className="p-2">
          <h3 className="text-xl font-bold text-black">{car.price} AZN ₼</h3>
          <h2 className="text-sm font-bold truncate w-64">{car.category}</h2>
...
<div className="max-w-[1200px] p-4 rounded-[8px] grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 mt-10 my-auto w-full bg-[#ffffff]">
        ...
        {posts?.map(post => (
          <Link key={post._id} to={`/elan/${post._id}`} >
            <div  className=" border sm:w-[240.4px] max-w-[240.4px] h-[300px] rounded-lg bg-white shadow-md hover:shadow-xl hover:scale-5 transition duration-50">
              <img
              ...
              <div className="p-4">
                ...
                <p className="capitalize text-gray-400 text-[16px]">{post.location}, {formatDate(post.data)} {getCurrentTime(post.data)} </p>
